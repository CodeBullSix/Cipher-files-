...
export const PrimaryDocumentViewer: React.FC<Props> = ({ document }) => {
  ...
  const getStampClass = (level: string) => {
    switch (level) {
      ...
      default: return 'text-cipher-accent border-cyan-500/80 bg-cyan-950/20';
    }
  };

  return (
    <div className="rounded-xl border border-slate-800 bg-cipher-surface overflow-hidden shadow-2xl">
      {/* Document Control Header */}
      <div className="px-4 py-3 bg-cipher-elevated border-b border-slate-800 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <FileText className="w-4 h-4 text-cipher-accent" />
        ...
        <div className="flex items-center gap-2">
          ...
          {document.redactedExcerpt && (
            ...
            >
              {showRedacted ? <Eye className="w-3.5 h-3.5 text-cipher-accent" /> : <EyeOff className="w-3.5 h-3.5 text-amber-400" />}
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded text-xs font-mono border transition-colors ${
              hasScanlines ? 'bg-cyan-950/40 text-cipher-accent-hover border-cipher-accent/40' : 'bg-slate-900 text-slate-400 border-slate-800'
            }`}
          >
      ...
      <div className={`relative p-6 sm:p-8 bg-cipher-panel border-y border-slate-800/60 font-mono text-slate-300 select-text overflow-hidden ${
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rotate-[-25deg] pointer-events-none opacity-10 select-none">
          <div className="text-6xl sm:text-8xl font-black border-8 border-cyan-400 text-cipher-accent px-8 py-4 uppercase tracking-widest">
        ...
        <div className="border-b border-dashed border-slate-800 pb-4 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          <div>
            ...
            <span className="text-cipher-accent-hover font-semibold">{document.originAgency}</span>
          </div>
          <div>
        ...
        <div className="bg-cipher-elevated border-l-2 border-cyan-500 p-3 rounded-r mb-6 text-xs text-slate-300">
          <span className="text-cipher-accent font-bold uppercase text-[10px] block mb-1">ARCHIVIST SUMMARY NOTE:</span>
          {document.summary}
        </div>
        ...
        <div className="space-y-4 text-xs sm:text-sm leading-relaxed text-slate-200">
          <div className="text-[11px] text-slate-400 border-b border-slate-800/80 pb-1 flex items-center justify-between">
            ...
            <span className="text-cipher-accent font-mono">OCR SCAN ACCURACY: 99.8%</span>
          </div>

          <div className="p-4 rounded bg-cipher-surface border border-slate-800 whitespace-pre-wrap font-mono leading-loose">
        ...
        <div className="mt-6 pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
          ...
          <span className="text-[10px] font-mono text-cipher-accent">HASH: SHA256-VERIFIED</span>
        </div>
      </div>
    </div>
  );
