...
export function TimelineView({ entityType, entityId, currentUser, onOpenEvent }: TimelineViewProps) {
  ...
  if (loading) return <div className="p-8 text-center"><Loader className="w-6 h-6 animate-spin mx-auto text-cipher-accent" /></div>;
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      {events.length === 0 ? (
        <div className="text-center py-12 border border-gray-800 rounded-xl bg-gray-900/30">
          ...
          <EmptyState icon={Calendar} title="NO EVENTS DOCUMENTED" message="The chronological timeline is currently empty for this case." />
        </div>
      ) : (
        <div className="relative pl-6 space-y-6 before:absolute before:inset-y-0 before:left-2.5 before:w-0.5 before:bg-gray-800">
          {events.map((event) => (
            <div key={event.id} className="relative">
              <div className="absolute -left-[30px] top-1.5 w-3 h-3 rounded-full bg-cipher-accent ring-4 ring-black" />
              <div className="p-4 bg-gray-900/50 border border-gray-800 rounded-lg hover:border-cyan-900/50 cursor-pointer transition-colors" onClick={(e) => { if ((e.target as any).closest('button')) return; sound.click(); onOpenEvent?.(event.id); }}>
                <div className="flex justify-between items-start mb-2">
                  <div className="flex flex-col">
                    <span className="text-cipher-accent font-mono text-sm font-bold">{event.dateString}</span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {(currentUser?.tier === 'ADMIN' || currentUser?.uid === event.createdBy) && (
                      <button onClick={() => { setEditingEvent(event); setIsModalOpen(true); sound.click(); }} className="text-gray-500 hover:text-cipher-accent ml-2">
                ...
                <div className="mt-3 pt-3 border-t border-gray-800 flex items-center justify-between">
                    ...
                    {(currentUser?.tier === 'ADMIN' || currentUser?.tier === 'MODERATOR' || currentUser?.uid === event.createdBy) && (
                      <button onClick={() => { setAttachEventId(event.id); sound.click(); }} className="text-xs text-cipher-accent hover:text-cipher-accent font-mono flex items-center gap-1">
