...
export const RabbitHoleGraph: React.FC<RabbitHoleGraphProps> = ({ 
  ...
  const handleTraceTargetSelected = async (targetNode: any) => {
    ...
    try {
      ...
      setAiBriefContent("### ⚠️ NEXUS TRACE FAILED\\n\\nCould not synthesize connection trail. Target classified.");
    } finally {
  ...
  const handleAIBrief = async () => {
    ...
    try {
      ...
      const result = await GeminiService.crossExamine({
        caseTitle: label,
        claim: \`Analyse the entity: \${label}\`,
        knownFacts: [\`Entity Type: \${selectedNode.type}\`],
        userHypothesis: "Provide a concise intelligence brief."
      });
      setAiBriefContent(result);
      onRewardXp(15, "Requested AI Brief");
    } catch (e: any) {
      setAiBriefContent("### ⚠️ AI BRIEF UNAVAILABLE\\n\\nIntelligence synthesis failed. Subject may be redacted.");
    } finally {
      setAiBriefLoading(false);
    }
  };
  ...
  return (
    <div className="flex-1 w-full h-full relative flex flex-col bg-[#050505] overflow-hidden rounded-xl border border-gray-800" ref={containerRef}>
      ...
      {selectedLink && (
        <div className="absolute top-20 right-4 z-30 w-80 border border-amber-500/50 bg-[#0D0D0D]/95 backdrop-blur-md p-4 shadow-2xl text-white animate-in slide-in-from-right-4 duration-150">
          <div className="flex items-center justify-between pb-2 mb-3 border-b border-white/10">
