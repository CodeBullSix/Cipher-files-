...
import_stmt = "import { ApiService } from '../services/apiService';\nimport { ArchiveEvidence } from '../types';\nimport { EvidenceDetailModal } from './EvidenceDetailModal';\n"
if "ApiService" not in content:
    content = content.replace("import { StatusBadge }", import_stmt + "import { StatusBadge }")
...
states = """
  ...
  useEffect(() => {
    ApiService.getEvidence(caseFile.id)
      .then(data => setCaseEvidence(data))
      .catch(err => console.error("Failed to load case evidence", err));
  }, [caseFile.id]);
"""
if "const [caseEvidence" not in content:
    content = content.replace("const [evidenceFilter, setEvidenceFilter]", states + "\n  const [evidenceFilter, setEvidenceFilter]")
...
content = re.sub(
    r"\{\/\* 3\. EVIDENCE \*\/}.*?\{activeTab === 'evidence' && \(.*?\)\}",
    evidence_tab_html.strip(),
    content,
    flags=re.DOTALL
)
